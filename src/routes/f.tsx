  ...
  return (
    <div className="min-h-screen bg-secondary/30 pb-10">
      ...
      <main className="mx-auto grid max-w-6xl gap-5 px-4 py-6 lg:grid-cols-[320px_1fr]">
        ...
        <FitterShop token={session.token} onSale={refresh} />
      </main>
    </div>
  );
}

function FitterShop({ token, onSale }: { token: string; onSale: () => Promise<void> }) {
  ...
  const confirm = async (product: CustomerProduct) => {
    try {
      const result = await confirmFn({ data: { token, productId: product.id } });
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {products.map((product) => <FitterProduct key={product.id} product={product} onConfirm={confirm} />)}
      </div>
    </section>
  );
}

function FitterProduct({ product, onConfirm }: { product: CustomerProduct; onConfirm: (product: CustomerProduct) => void }) {
  return (
    <div className="rounded-2xl border border-border bg-card p-4 shadow-soft">
      ...
      <div className="mt-3 space-y-2">
        ...
        <Button className="w-full gap-2" onClick={() => onConfirm(product)}>
